...
const BACK_X_OFF   = 704;   // x-offset of Male Back in sprite

// ── Layout constants ──────────────────────────────────────────────────────────
// Body paths are drawn in a 140 × 290 space (matches BodyMap FIGURE_LOCAL_*).
const VB_W = 140;
const VB_H = 290;
// Each individual silhouette is rendered at this size
const COL_W    = 72;
const COL_H    = Math.round(COL_W * (VB_H / VB_W)); // ≈ 149, preserves aspect ratio
const SILO_GAP = 4;   // gap between front and back silhouettes within one session
const PAIR_W   = COL_W * 2 + SILO_GAP; // width of one session pair (front+back)
const SESS_GAP = 40;  // horizontal gap between session pairs
...
/** Final SVG x for a cx in body-space for the given session column and view */
function fx(cx: number, colIdx: number, view: "front" | "back"): number {
  const pairStart = colIdx * (PAIR_W + SESS_GAP);
  const siloOffset = view === "back" ? COL_W + SILO_GAP : 0;
  return pairStart + siloOffset + cx * SX;
}
...
const ConnectedTimeline = memo(function ConnectedTimeline({ sessions }: Props) {
  ...
  const totalW = sessions.length * PAIR_W + (sessions.length - 1) * SESS_GAP;
  const totalH = COL_H + DATE_H;

  // ── Build region:view → occurrence list (one entry per session it appears in) ──
  const regionMap = new Map<string, Occurrence[]>();

  sessions.forEach((session, colIdx) => {
    const seenInSession = new Set<string>();
    session.spots.forEach((spot) => {
      const view = spot.view ?? "front";
      const key = `${spot.label}:${view}`;
      if (seenInSession.has(key)) return;
      seenInSession.add(key);
      const list = regionMap.get(key) ?? [];
      list.push({ colIdx, cx: spot.cx, cy: spot.cy, intensity: spot.intensity, view });
  ...
  return (
    <div className="relative">
      ...
      <div className="overflow-x-auto pb-1">
        ...
        >
          {/* ── Body silhouettes: Male Front + Male Back per session ── */}
          {sessions.map((_, colIdx) => {
            ...
            return (
              <g key={`sil-${colIdx}`}>
                ...
                {/* Posterior (back) */}
                <svg
                  x={pairStart + COL_W + SILO_GAP}
                  y={0}
                  width={COL_W}
                  height={COL_H}
                  viewBox={`${BACK_X_OFF} ${CROP_Y} ${SLICE_W} ${CROP_H}`}
                  preserveAspectRatio="xMidYMid meet"
                  overflow="hidden"
                >
                  <image
          ...
          {connectedRegions.map(([regionKey, occs]) => {
            ...
            return occs.slice(0, -1).map((occ, segIdx) => {
              ...
              const x1 = fx(occ.cx, occ.colIdx, occ.view);
              const y1 = fy(occ.cy);
              const x2 = fx(next.cx, next.colIdx, next.view);
              const y2 = fy(next.cy);
          ...
          {sessions.map((session, colIdx) =>
            session.spots.map((spot, dotIdx) => {
              const view = spot.view ?? "front";
              const x = fx(spot.cx, colIdx, view);
              const y = fy(spot.cy);
              const r = spot.size === "pinpoint" ? 2.5 : spot.size === "regional" ? 4.5 : 6.5;
              const color = dotColor(spot.intensity);
              const regionKey = `${spot.label}:${view}`;
              const isOnHoveredLine = hoveredRegion === regionKey;
              return (
                <circle
          ...
          {/* ── Session date labels ── */}
          {sessions.map((session, colIdx) => {
            const labelX = colIdx * (PAIR_W + SESS_GAP) + PAIR_W / 2;
            return (
              <text
                key={`date-${colIdx}`}
                x={labelX}
